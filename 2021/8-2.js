...
const rawInput = `aecgdbf badcg fbcage gdabce be gaedb bced dfeag adbfgc abe | gedbacf be bdfcga bedfgca
...
bgfd cbafegd dcabe gfcba dagefc gda ecfabg dg bcadfg bcgad | caebd cdgba acgdef gfecbda`.split('\n');
...
const checkNumber = (input) => {
  ...
  Object.keys(numbers).forEach((key) => {
    if (input.length === numbers[key].length && input.split('').every(x => numbers[key].includes(x))) {
      returnValue = parseFloat(key);
    }
  })
  return returnValue;
}
...
const testRow = (input, index) => {
  input.forEach((inputRow) => {
    ...
    const dataValues = data[1].split(' ');
    const map = {};
    let output = '';
    while (Object.keys(map).length < 7) {
    ...
    dataValues.forEach((data) => {
      const fixedValue = data.split('').map(x => (map[x] || x)).join('');
      const number = checkNumber(fixedValue);
      if (number === -1) {
        console.log(map);
        console.log(data, fixedValue);
      }
      output += number;
    })
    // console.log(output);
    // outputs.push(parseFloat(output))
  });
}
