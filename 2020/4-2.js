...
const smallInput = `pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
const splitInput = input.split(new RegExp('\n\n', 'g'));
...
const validations = {
  byr: (value) => {
    return validateYear(1920, 2002, parseFloat(value));
  },
  iyr: (value) => {
    return validateYear(2010, 2020, parseFloat(value));
  },
  eyr: (value) => {
    return validateYear(2020, 2030, parseFloat(value));
  },
  hgt: (value) => {
    // (Height) - a number followed by either cm or in:
    // If cm, the number must be at least 150 and at most 193.
    // If in, the number must be at least 59 and at most 76.
    const unit = value.slice(-2).trim();
    if ((unit !== 'cm' && unit !== 'in')) return false;
    const length = value.replace(unit, '');
    if (unit === 'cm') {
  ...
  hcl: (value) => {
    ...
    console.log(val);
    return new RegExp(/[0-9a-f]/).test(val);
  },
  ecl: (value) => {
...
const validatePassportData = (keyValue) => {
  const split = keyValue.split(':');
  return validations[split[0]](split[1]);
}

const isValidPassport = (passport) => {
  const fixedPassport = passport.replace(/\n/g, ' ');
  if (!requiredFields.every(x => fixedPassport.includes(x))) return false;
  const splitPassport = fixedPassport.split(' ');
  let isValid = true;
  splitPassport.forEach((sp) => {
    if (!validatePassportData(sp)) {
      // console.log(sp)
      isValid = false;
    }
  });
...
splitInput.forEach((passport) => {
  if (isValidPassport(passport)) {
    validPassports += 1;
  }
})

console.log(splitInput.length, validPassports);