...
// 6
let totalSteps = 0;
const setNewPosition = (cable, movement) => {
	if(!movement) return;
	
	let position = cable.position;
	let x = cable.position[0];
	let y = cable.position[1];
	
	const direction = movement[0];
	const steps = parseFloat(movement.substring(1));
	for(let i = 0; i < steps; i += 1) {
		totalSteps += 1;
		 ...
		 cable.steps[`${x}:${y}`] = {steps: totalSteps, coordinates: `${x}:${y}`};
	}
	
	cable.position = [x,y];
}

for(let i = 0; i < cable1.directions.length; i += 1) {
	setNewPosition(cable1, cable1.directions[i]);
}

totalSteps = 0;
for(let i = 0; i < cable2.directions.length; i += 1) {
	setNewPosition(cable2, cable2.directions[i]);
}

//console.log(cable1, cable2);
const test = [];
Object.keys(cable1.steps).forEach((step) => {
	const val = cable2.steps[step];
	if(val) {
	console.log(val, step);
		val.steps += cable1.steps[step].steps;
		test.push(val);
	}
});
...
lowest = -1;
test.sort((a, b) => a.steps - b.steps)
console.log(test);
test.forEach((te) => {
	const value = te.coordinates.split(':');

	const dist = distance([0, 0], [parseFloat(value[0]), parseFloat(value[1])]);
	if(lowest === -1 || lowest > dist) {
		lowest = dist;
	}
});
