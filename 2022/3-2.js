...
const getPriority = (data) => {
  const index = letters.indexOf(data);
  if(index > -1) return (index + 1);
  return (letters.indexOf(data.toLowerCase()) + 27);
}

const getAuthenticitySticker = (group) => {
  let authenticitySticker;
  group[0].split('').forEach((l) => {
    if(group.every((g) => g.includes(l))) {
      authenticitySticker = l;
    }
  });
  return getPriority(authenticitySticker);
};

let totalValue = 0;

for(let i = 0; i < fixedInput.length; i += 3) {
  totalValue += getAuthenticitySticker(fixedInput.slice(i, (i +3)));
}
