...
const getContent = (data) => {
  ...
  const compA = data.substring(0, len).split('');
  const compB = data.substring(len);
  
  const matches = [];

  compA.forEach((l) => {
    const index = compB.indexOf(l);
    if(index > -1 && !matches.includes(l)) {
      matches.push(l);
    }
  })
...
let totalValue = 0;
fixedInput.forEach((r) => {
  totalValue += getContent(r);
});
