const fullData =
  ...
  Game 100: 2 green, 1 blue; 9 red, 8 green, 1 blue; 4 red, 10 green, 1 blue; 17 green, 8 red; 5 green, 1 blue, 7 red; 14 red, 12 green`.split(
    "\n"
  );
...
const isRoundValid = (input) => {
  ...
  input.forEach((d) => {
    const tries = d.split(",").map((x) => x.trim());

    tries.forEach((t) => {
      const [count, color] = t.split(" ");
      const colorRule = biggestSmallest[color.replace(",", "")];
      if (parseFloat(count) > colorRule) {
        biggestSmallest[color.replace(",", "")] = parseFloat(count)
      }
    });
  });

  return Object.values(biggestSmallest).reduce((a, b) => a * b, 1);
};

fullData.forEach((gr) => {
  const row = gr.split(":");
  const data = row[1].split(";").map((x) => x.trim());
  total += isRoundValid(data);
});
