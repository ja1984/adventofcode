...
8hl5eight`
  .split("\n")
  .map(x => replaceWithDigits(x))
  .map((x) => x.replace(/\D/g, ""))
  .map(x => parseFloat(`${x.at(0)}${x.at(-1) ?? x.at(0)}`));


console.log(testData.reduce((a, b) => a + b, 0));
